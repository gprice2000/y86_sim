...
ConditionCodes * ConditionCodes::getInstance()
{
   if (ccInstance == NULL)
   {
      // ConditionCodes newInstance;

      // ccInstance = &newInstance;
...
// //Use your getBits in Tools.C.
   ...
   if (ccNum == OF)
   {
      result = Tools::getBits(codes,OF,OF);
   ...
   if (ccNum == SF)
   {
      ...
      result = Tools::getBits(codes,SF,SF);
   }

   if (ccNum == ZF )
   {

      result = Tools::getBits(codes,ZF,ZF);
   }
   return result;
...
void ConditionCodes::dump()
{
   ...
   int32_t sf = Tools::getBits(codes, SF, SF);//Load up
