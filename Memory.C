...
//GROUP PriceKamto - Gabe Price, Kevin Kamto
...
{
   if (address % 8 != 0 || address >= 0x1000 || address < 0){
      imem_error = true;
      return 0;
   }
   uint8_t bytes[8];
   ...
   uint64_t value = Tools::buildLong(bytes);
   imem_error = false;
   //printf("%lx", value);
//   uint64_t  value = mem[address];
   return value;
}
...
void Memory::putLong(uint64_t value, int32_t address, bool & imem_error)
{
   ...
   if (address % 8 != 0 || address >= 0x1000 || address < 0x0){
      imem_error = true;
      return;
   }
   imem_error = false;
   ...
   while(j <= 7){
      ...
      mem[address] = Tools::getByte(value,j);
      address++ ; 
      // i++;
      j++;
